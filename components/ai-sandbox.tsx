    ...
    <div className="p-8 rounded-3xl bg-slate-900 border border-slate-800 text-white space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        {presets.map((p, i) => (
          <button
            ...
            className="text-[11px] font-mono bg-slate-800 hover:bg-cyan-950 hover:text-cyan-300 text-slate-300 px-3 py-1.5 rounded-lg border border-slate-700 transition-colors text-left"
          >
            &gt; {p}
          </button>
        ))}
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          disabled={loading}
          className="px-5 py-2.5 bg-cyan-600 hover:bg-cyan-500 text-white rounded-xl text-xs font-semibold uppercase tracking-wider transition-colors disabled:opacity-50"
        >
      ...
      {/* Output Console */}
      {response && (
        <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 text-xs font-mono text-cyan-300 animate-hero-entry leading-relaxed">
          {response}
        </div>
      )}
    </div>
