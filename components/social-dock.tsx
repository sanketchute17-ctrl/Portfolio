...
export function SocialDock() {
  ...
  return (
    <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 z-40">
      <div className="bg-slate-900/90 backdrop-blur-xl px-4 py-2.5 rounded-full flex items-center space-x-3 shadow-2xl border border-slate-700/80 hover:scale-[1.02] transition-transform">
        {socialLinks.map((item, idx) => (
          <a
            ...
            className="p-2 rounded-full hover:bg-slate-800 text-slate-300 hover:text-orange-400 transition-colors focus:outline-none focus:ring-2 focus:ring-orange-500 relative group"
          >
            {item.icon}
            {/* Tooltip */}
            <span className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-2.5 py-1 text-[10px] font-mono font-semibold text-slate-100 bg-slate-950 rounded-md opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none shadow-md border border-slate-800">
              {item.name}
            </span>
          </a>
        ))}
